...
Undeploy Nondeployed XApp And Expect Error
     ...
     @{dNames} =          Pluck      Name  ${d}
     @{xapp} =            Evaluate   random.choice(filter(lambda v: v not in ${dNames}, ${a}))  random
     ${status} =          Run Keyword And Ignore Error  Undeploy XApp Via AppMgr  ${xapp}
     Should Be Equal As Strings  ${status}          FAIL
...
Request Nonexistent XApp And Expect Error
     ...
     ${xapp} =        Generate UUID
     ${status} =      Get XApp By Name From AppMgr  {$xapp}
     Should Be Equal As Strings         ${status}   FAIL
