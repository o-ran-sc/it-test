...
func (r robotXApp) CreatePolicy(t int, i string, policy robotPolicy)  {
  ...
  /* sdlgo populates zero (nil) values for nonexistent keys */
  existingPolicy, policyExists := policies[k]

  if !policyExists || existingPolicy == nil {
    ...
    xapp.Logger.Error("Attempt to recreate existing instance %s of policy %d", i, t)
    r.metrics["DuplicatePolicyCreates"].Inc()
    r.SendA1Response(t, i, "robot", "ERROR")
  }  
}

func (r robotXApp) UpdatePolicy(t int, i string, policy robotPolicy)  {
  k := sdlKey(t, i)
  policies, _  := xapp.Sdl.Read(k)
  ...
  if existingPolicy != nil {
    err := xapp.Sdl.Store(k, policy.Message)
    if err == nil {
      xapp.Logger.Debug("Updated instance %s of policy %d, old value: %s, new value: %s",
                        i, t, existingPolicy, policy.Message)
      r.metrics["PolicyUpdates"].Inc()
      r.metrics["dbStores"].Inc()
      r.SendA1Response(t, i, "robot", "OK")
    } else {
      xapp.Logger.Error("Failed to update DB record for instance %s of policy %d: %v", i, t, err)
      r.metrics["dbStoreFailures"].Inc()
      r.SendA1Response(t, i, "robot", "ERROR")
    }
  } else {
    xapp.Logger.Error("Attempt to delete nonexistent instance %s of policy %d", i, t)
    r.metrics["NonexistentPolicyUpdates"].Inc()
    r.SendA1Response(t, i, "robot", "ERROR")
  }  
}
...
func (r robotXApp) Consume(msg *xapp.RMRParams) (err error) {
  ...
  case A1_POLICY_REQ:
    ...
    case "UPDATE":
      go r.UpdatePolicy(a1Msg.Type, a1Msg.Instance, a1Msg.Payload)
    case "DELETE":
